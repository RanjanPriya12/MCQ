...
router.post("/createQuestions", authenticate, async (req, res) => {
  try {
    ...
    if (userIdentity.name !== "admin") {
      return res
        .status(400)
        .json({ errors: [{ msg: "Only admin can generate question" }] });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (userIdentity.name !== "admin") {
        return res
          .status(400)
          .json({ errors: [{ msg: "Only admin can delete  questions" }] });
      }
...
router.put('/update/:quesId',authenticate,async(req,res)=>{
  try {
    ...
    if(userRole.name === "user") {
      return res
          .status(400)
          .json({ errors: [{ msg: "Only admin have right to update the questions" }] });
    }
